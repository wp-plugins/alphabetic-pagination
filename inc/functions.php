<?php
	...
	function ap_menu()
	{
		...
		$title = 'Alphabetic Pagination'.($ap_customp?' Pro':'');
...
$alphabetz_bar .= '<li class="ap_numeric">';
			...
			switch($taxonomy){
				...
				case 'category':
					...
					if(!empty($ap_tax_types)){
						$response = (in_array($current_cat, $ap_tax_types) && (is_array($ap_tax_types_x) && !empty($ap_tax_types_x) && !in_array($current_cat, $ap_tax_types_x)));
					}
	...
	if(!function_exists('ap_where_clause')){
		function ap_where_clause($where=''){
			...
			global $wpdb;
			global $ap, $ap_customp, $ap_queries, $ap_query;
	...
	function ap_plugin_links($links) { 
		...
		if($ap_customp){
			...
			$ap_premium_link = '<a href="'.$ap_premium_link.'" title="Go Premium" target=_blank>Go Premium</a>'; 
